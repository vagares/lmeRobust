...
scenarios_CI=scenarios[(scenarios$alphac==10)&(scenarios$CCMind==1),]

if (max(scenarios_CI$pe)!=0){
  pname="pe";
  size=unique(scenarios_CI$mec);
  levelsvec=pevec}
if (max(scenarios_CI$pb)!=0){
  pname="pb";
  size=unique(scenarios_CI$mbc2);
  levelsvec=pbvec}
if (max(scenarios_CI$px)!=0){
  pname="px";
  size=unique(scenarios_CI$alphac);
  levelsvec=pxvec}
...
for (i in 1:nrow(scenarios_CI)){
  ...
  if (CCMindsample==FALSE){stop("Contains ICM scenarios_CI")}
  ...
  for (j in 1:nrep){
    ...
    lwbbetaMLE=MLESMMcTAUind$MLE$beta[j,]-qnorm(0.975)*sqrt(diag(MLESMMcTAUind$MLE$varbeta[[j]]))
    upbetaMLE=MLESMMcTAUind$MLE$beta[j,]+qnorm(0.975)*sqrt(diag(MLESMMcTAUind$MLE$varbeta[[j]]))
  ...
  for (j in 1:nrep){
    ...
    lwbbetaS=MLESMMcTAUind$S$beta[j,]-qnorm(0.975)*sqrt(diag(MLESMMcTAUind$S$varbeta[[j]]))
    upbetaS=MLESMMcTAUind$S$beta[j,]+qnorm(0.975)*sqrt(diag(MLESMMcTAUind$S$varbeta[[j]]))
  ...
  for (j in 1:nrep){
    ...
    lwbbetaMM=MLESMMcTAUind$MM$beta[j,]-qnorm(0.975)*sqrt(diag(MLESMMcTAUind$MM$varbeta[[j]]))
    upbetaMM=MLESMMcTAUind$MM$beta[j,]+qnorm(0.975)*sqrt(diag(MLESMMcTAUind$MM$varbeta[[j]]))
  ...
  for (j in 1:nrep){
    ...
    lwbthetaMLE=MLESMMcTAUind$MLE$theta[j,]-qnorm(0.975)*sqrt(diag(MLESMMcTAUind$MLE$vartheta[[j]]))
    upbthetaMLE=MLESMMcTAUind$MLE$theta[j,]+qnorm(0.975)*sqrt(diag(MLESMMcTAUind$MLE$vartheta[[j]]))
  ...
  for (j in 1:nrep){
    ...
    lwbthetaS=MLESMMcTAUind$S$theta[j,]-qnorm(0.975)*sqrt(diag(MLESMMcTAUind$S$vartheta[[j]]))
    upbthetaS=MLESMMcTAUind$S$theta[j,]+qnorm(0.975)*sqrt(diag(MLESMMcTAUind$S$vartheta[[j]]))
  ...
  for (j in 1:nrep){
    ...
    lwbthetaMM=MLESMMcTAUind$MM$theta[j,]-qnorm(0.975)*sqrt(diag(MLESMMcTAUind$MM$vartheta[[j]]))
    upbthetaMM=MLESMMcTAUind$MM$theta[j,]+qnorm(0.975)*sqrt(diag(MLESMMcTAUind$MM$vartheta[[j]]))
...
titlename=paste0("CI CCM: rc=",unique(scenarios_CI[,10]),
                 ", n=",unique(scenarios_CI[,2]),
...
plotCPbeta2=ggplot(CP_MLESMMbeta,
                   aes(x=get(pname),y=CovPrbeta2,group=Estimator,color=Estimator))+
  geom_line()+
  xlab("Fixed % of contaminated cases")+
  ylab(quote(beta[2]))+
  ggtitle(titlename)+
