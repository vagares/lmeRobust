...
mecvec=c(-40,-80,-160,-300)     # size of contamination-shift in error
...
for (cs in 2:2){
  ...
  if (cs==1) {CCMind=FALSE;Sclaudio=FALSE;randcont=1;Xa=FALSE;Xshiftall=FALSE} # value of randcont has no influence
  if (cs==2) {CCMind=TRUE;Sclaudio=FALSE;randcont=1;Xa=FALSE;Xshiftall=FALSE}
  if (cs==3) {CCMind=TRUE;Sclaudio=FALSE;randcont=0;Xa=FALSE;Xshiftall=FALSE}
  if (cs==4) {CCMind=TRUE;Sclaudio=FALSE;randcont=1;Xa=TRUE;Xshiftall=FALSE}
  if (cs==5) {CCMind=TRUE;Sclaudio=FALSE;randcont=1;Xa=TRUE;Xshiftall=TRUE}
  if (cs==6) {CCMind=TRUE;Sclaudio=TRUE;randcont=1;Xa=TRUE;Xshiftall=TRUE}
    # ONLY pevec varying
   ...
   for (i in 1:length(pxvec)){if (Xa==FALSE)
      {for (j in 1:length(alphacvec)){
       ...
       scenarios=rbind(scenarios,c(nrep,n,k,0,0,pxvec[i],0,0,0,
                                   randcont,as.integer(cTAUind),as.integer(CCMind),
                                   as.integer(Xa),as.integer(Xshiftall),muxvec[j],as.integer(Sclaudio)))
     } # END j-loop alphacvec
     }
    } # END i-loop pxvec
  }
