...
scenarios_boxplot=scenarios[(scenarios$px==0.10)&(scenarios$CCMind==1),]


if (max(scenarios_boxplot$mec)!=0){
  pname="mec";
  perc=unique(scenarios_boxplot$pe);
  levelsvec=mecvec}
if (max(scenarios_boxplot$mbc2)!=0){
  pname="mbc2";
  perc=unique(scenarios_boxplot$pb);
  levelsvec=mbc2vec}
if (max(scenarios_boxplot$alphac)!=1){
  pname="alphac";
  perc=unique(scenarios_boxplot$px);
  levelsvec=alphacvec}
...
for (i in 1:nrow(scenarios_boxplot)){
  ...
  if (scenarios_boxplot$alphac[i]!=1){
...
plotnoei=ggplot(boxplotOUTLIER,aes(x=get(pname),y=noei/nsample))+
  geom_boxplot()+
  xlab("Size of contamination")+
...
plotnoe=ggplot(boxplotOUTLIER,aes(x=get(pname),y=noe/(nsample*ksample)))+
  geom_boxplot()+
  xlab("Size of contamination")+
